...
function OnAnyChatEnterRoom(dwRoomId, errorcode) {
    ...
    if (errorcode == 0) {
		...
		ShowNotifyMessage(username+" ,你好!欢迎来到怡成网络医院咨询互动交流区", NOTIFY_TYPE_SYSTEM);
        // 设置本地视频显示位置
        BRAC_SetVideoPos(mSelfUserId, GetID("AnyChatLocalVideoDiv"), "ANYCHAT_VIDEO_LOCAL");
		GetID("ANYCHAT_VIDEO_LOCAL").SetBkImage(mDefaultAdsAddr);
        // 设置远程视频显示位置（没有关联到用户，只是占位置）
        BRAC_SetVideoPos(0, GetID("AnyChatRemoteVideoDiv"), "ANYCHAT_VIDEO_REMOTE");
		GetID("ANYCHAT_VIDEO_REMOTE").SetBkImage(mDefaultAdsAddr);
...
function OnAnyChatUserAtRoom(dwUserId, bEnterRoom) {
    ...
    if (bEnterRoom == 1) {
		ShowNotifyMessage(BRAC_GetUserName(dwUserId) +"&nbsp进入了视频互动交流区!", NOTIFY_TYPE_NORMAL);
		if(mTargetUserId == -1)						// 默认打开一个用户的音视频
			RequestOtherUserVideo(dwUserId);
    }
    else {
		ShowNotifyMessage(BRAC_GetUserName(dwUserId) +"&nbsp离开了视频互动交流区!", NOTIFY_TYPE_NORMAL);
...
function OnAnyChatCameraStateChange(dwUserId, State) {
    ...
    if (State == 1) GetID(dwUserId + "_CameraTag").src = "http://api.yicheng120.com/Public/Video/images/advanceset/camera_false.png";
    if (State == 2) GetID(dwUserId + "_CameraTag").src = "http://api.yicheng120.com/Public/Video/images/advanceset/camera_true.png";
