...
function DisplayTextMessage(fromuserid, message) {
	...
	if(fromuserid==mSelfUserId){
		namestr = namestr.fontcolor("#25bff7");
		
		msgdiv.setAttribute("class", "TheMsgStyle2");
	}else{
	...
	msgdiv.innerHTML = namestr + "<BR />" + message;
...
function ShowNotifyMessage(message, type) {
    if (type == NOTIFY_TYPE_SYSTEM) {
        message = message.fontcolor("#000000");
    } else {
        message = message.fontcolor("#333333");
    }
	var msgdiv = document.createElement("div");
    msgdiv.setAttribute("class", "TheMsgStyle");
	msgdiv.innerHTML = '<div style="font-size:14px; text-align:center;">----'+message+'----</div>';
 	//msgdiv.innerHTML = '<span style="font-size:14px;">【 '+message + "&nbsp&nbsp" + GetTheTime().fontcolor("#999999")+'】</span><BR />';
    GetID("ReceiveMsgDiv").appendChild(msgdiv);
    DisplayScroll("ReceiveMsgDiv");
}
...
function RequestOtherUserVideo(userid) {
    ...
    for (var j = 0; j < userimgobj.length; j++) {
        if (userimgobj[j].getAttribute("class") == "MicrophoneTag") { // 该图片为 话筒 图片
            userimgobj[j].src = "http://api.yicheng120.com/Public/Video/images/advanceset/microphone_false.png";
    ...
    GetID(userid + "_MicrophoneTag").src = "http://api.yicheng120.com/Public/Video/images/advanceset/microphone_true.png"; // 点亮话筒图片
...
function RoomUserListControl(userid, bInsert) {
    ...
    if (bInsert) {
        ...
        if (BRAC_GetCameraState(userid) == 1)
            AddImage(itemdiv, userid + "_CameraTag", "CameraTag", "http://api.yicheng120.com/Public/Video/images/advanceset/camera_false.png", userid); // 添加摄像头图片<img>标签
        if (BRAC_GetCameraState(userid) == 2)
            AddImage(itemdiv, userid + "_CameraTag", "CameraTag", "http://api.yicheng120.com/Public/Video/images/advanceset/camera_true.png", userid); // 添加摄像头图片<img>标签
        // 判断当前ID是否为自己
        if (userid == mSelfUserId) {
            AddImage(itemdiv, mSelfUserId + "_MicrophoneTag", "mSelfMicrophoneTag", "http://api.yicheng120.com/Public/Video/images/advanceset/microphone_true.png", userid); // 添加话筒图片<img>标签
            itemdiv.innerHTML += "&nbsp" + BRAC_GetUserName(mSelfUserId) + "(自己)";
        } else {
            AddImage(itemdiv, userid + "_MicrophoneTag", "MicrophoneTag", "http://api.yicheng120.com/Public/Video/images/advanceset/microphone_false.png", userid); // 添加话筒图片<img>标签
            // 添加用户姓名<a>标签
...
function MicrophoneOnclick(userid) {
    ...
    GetID(userid + "_MicrophoneTag").onclick = function () { // 话筒点击事件
        ...
        if (ImgPath[ImgPath.length - 1] == "microphone_true.png") {
            GetID(userid + "_MicrophoneTag").src = "http://api.yicheng120.com/Public/Video/images/advanceset/microphone_false.png";
            BRAC_UserSpeakControl(userid, 0); // 关闭语音
        }
        else {
            GetID(userid + "_MicrophoneTag").src = "http://api.yicheng120.com/Public/Video/images/advanceset/microphone_true.png";
