...
abstract class OAuth2 {
    ...
    public function verify_access_token($scope = null, $exit_not_present = true, $exit_invalid = true, $exit_expired = true, $exit_scope = true, $realm = null) {
        ...
        if ($token_param === false){ // Access token was not provided
            return $exit_not_present ? $this->error(ERROR_BAD_REQUEST, ERROR_INVALID_GRANT) : FALSE;

            // 修改原有header传输为返回json格式传输
        ...
        if (isset($token["expires"]) && time() > $token["expires"]){
            return $exit_expired ?$this->error(ERROR_BAD_REQUEST, ERROR_EXPIRED_TOKEN) : FALSE;
        ...
        if ($scope &&
            ...
            return $exit_expired ?$this->error(ERROR_BAD_REQUEST, ERROR_USER_DENIED) : FALSE; 
        ...
        // 动态判断用户权限
        $cinfo  =   $this->get_client_info($token['client_id']);
        if ( !$cinfo || $cinfo['islock'] == 1 ) {
            oauthjson(10002,'没有权限');
        }

        // 统计用户调用次数
        $check_frequency    =   $this->frequency($token['client_id']);
        if ( $check_frequency === FALSE ) {
             $this->error(ERROR_BAD_REQUEST, 'Over the request at this time');
        }
    ...
    private function get_access_token_param() {
        ...
        if ($auth_header !== false) {
            ...
            if (preg_match('/\s*Token\s*="(.+)"/', $auth_header, $matches) == 0 || count($matches) < 2)
                $this->error(ERROR_BAD_REQUEST, ERROR_INVALID_REQUEST);
    ...
    public function grant_access_token() {
        ...
        // 判断如果为手机应用，不需要判断 redirect_uri
        $client_type    =   $this->get_client_type($client[0]);

        // Do the granting
        switch ($input["grant_type"]) {
            case AUTH_CODE_GRANT_TYPE:
                if ( !$input["code"] || (!$input["redirect_uri"] && $client_type != 2 ) )
                    $this->error(ERROR_BAD_REQUEST, ERROR_INVALID_REQUEST);

                $stored = $this->get_stored_auth_code($input["code"]);

                if ($stored === null || ($input["redirect_uri"] != $stored["redirect_uri"] && $client_type != 2 ) || $client[0] != $stored["client_id"])
        ...
        // 判断当天 token 获取次数
        $checktokentimes=   $this->get_today_token_time($client[0]);
        if ( $checktokentimes >= 1000 ) {
            oauthjson(10003,'获取频率超出极限');
            exit;
        }
        ...
        // 删除code信息
        if( $token ){
            $this->delete_auth_code($input["code"],$stored['client_id']);
        }
    ...
    public function post_authorize_params() {
        $filters = array(
            "client_id" => array("filter" => FILTER_VALIDATE_REGEXP, "options" => array("regexp" => REGEX_CLIENT_ID), "flags" => FILTER_REQUIRE_SCALAR),
            "response_type" => array("filter" => FILTER_VALIDATE_REGEXP, "options" => array("regexp" => REGEX_AUTH_RESPONSE_TYPE), "flags" => FILTER_REQUIRE_SCALAR),
            "redirect_uri" => array("filter" => FILTER_VALIDATE_URL, "flags" => array(FILTER_FLAG_SCHEME_REQUIRED, FILTER_REQUIRE_SCALAR)),
    ...
    public function finish_client_authorization($is_authorized, $type, $client_id, $redirect_uri, $state, $scope = null) {
        ...
        $this->do_redirect_uri_json( $result['query'] );
    }
    ...
    private function do_redirect_uri_json( $result) {
    ...
    private function create_access_token($client_id, $scope) {
        //  获取token有效期（ 单位：s ）
        $expires_in =   $this->get_token_time( $client_id , $this->access_token_lifetime );
        ...
        $this->store_access_token($token["access_token"], $client_id, time() + $expires_in, $scope);
    ...
    private function gen_access_token() {
        return md5(base64_encode(pack('N6', mt_rand(), mt_rand(), mt_rand(), mt_rand(), mt_rand(), mt_rand())));
