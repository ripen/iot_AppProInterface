 ...
 * @version V1.0
 */
class OxController extends BaseController {  
    ...
    public function index(){
        ...
        $info   =   D('Ox')->lists($drugsid,$p,$pagesize);

        if ( !$info ) {
            oauthjson(40001,'未获取到数据');
            exit;
        }
    ...
    public function lists(){
        ...
        // 查询用户数据列表信息 
        $info   =   D('Ox')->listsbycard($cardinfo['id'],$p,$pagesize);
        if ( !$info ) {
            oauthjson(40001,'未获取到数据');
            exit;
        }
    ...
    public function show(){
        ...
        $info   =   D('Ox')->getinfo($id);

        if ( !$info ) {
            oauthjson(40002,'未查询到数据');
            exit;
        }
        ...
        $postdata               =   array();
        $postdata['pr']         =   $info['pr'];
        $postdata['saturation'] =   $info['saturation'];

        $analysis   =   new \Oauth\Common\analysis();
        $result     =   $analysis->result($postdata,'ox');
        
        if ( !$result || $result['status'] == 'resultError' ) {
            oauthjson(40003,'未分析出结果');
            exit;
        }
