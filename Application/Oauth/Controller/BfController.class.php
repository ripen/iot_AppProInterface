...
class BfController extends BaseController {  
    ...
    public function index(){
        ...
        $info   =   D('Bf')->lists($drugsid,$p,$pagesize);

        if ( !$info ) {
            oauthjson(50001,'未获取到数据');
            exit;
        }
    ...
    public function lists(){
        ...
        // 查询用户数据列表信息 
        $info   =   D('Bf')->listsbycard($cardinfo['id'],$p,$pagesize);
        if ( !$info ) {
            oauthjson(50001,'未获取到数据');
            exit;
        }
    ...
    public function show(){
        ...
        $info   =   D('Bf')->getinfo($id);

        if ( !$info ) {
            oauthjson(50002,'未查询到数据');
            exit;
        }
        ...
        $postdata['tg']     =   $info['tg'];
        $postdata['tc']     =   $info['tc'];
        $postdata['ltc']    =   $info['ltc'];
        $postdata['htc']    =   $info['htc'];

        $analysis   =   new \Oauth\Common\analysis();
        $result     =   $analysis->result($postdata,'bf');
        
        if ( !$result || $result['status'] == 'resultError' ) {
            oauthjson(50003,'未分析出结果');
            exit;
        }
