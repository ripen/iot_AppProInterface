...
class WeModel extends Model {  
	...
	public function lists( $drugs = array(),$p = 1 , $pagesize = 10 ){
		...
		$info 	=	$db->field('id,weight,bmi,bf,protein,watercontentrate,muscle,mineralsalts,fat,cardid,examtime')->where($map)->limit($curpage.','.$pagesize)->order('createtime desc')->select();
	...
	public function getinfo( $id = 0 ){
		...
		$info 	=	M('kangbao_humanbody')->where(array('id'=>$id))->find();
