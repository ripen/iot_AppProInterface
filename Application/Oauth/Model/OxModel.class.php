...
class OxModel extends Model {  
	...
	public function lists( $drugs = array(),$p = 1 , $pagesize = 10 ){
		...
		$info 	=	$db->field('id,pr,saturation,cardid,examtime')->where($map)->limit($curpage.','.$pagesize)->order('createtime desc')->select();
	...
	public function listsbycard( $cardid = 0,$p = 1 , $pagesize = 10){
		...
		$info 	=	$db->field('id,pr,saturation,examtime')->where($map)->limit($curpage.','.$pagesize)->order('createtime desc')->select();
	...
	public function getinfo( $id = 0 ){
		...
		$info 	=	M('kangbao_oxygen')->where(array('id'=>$id))->find();
