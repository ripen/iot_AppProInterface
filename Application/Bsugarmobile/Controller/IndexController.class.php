...
class IndexController extends BsugarController {
	...
	public function index() {
		...
		$cardstatus = '';
		$cardstatus = \Bsugarmobile\Model\Drug_card_userModel::getcheckcard ($userid,$Infoarr ['cardid']);
		...
		$Insertdatas = Array (
				...
				'attr' => $Infoarr ['attrs'],
				'sn' => $Infoarr ['sn'],
				'examtime' => date('Y-m-d H:i:s',time()),
		);
		if ($Insertdatas['attr'] == 4) {
			$Insertdatas['attr'] =	2;
		}
	...
	private function updatetime($userid=0){
		if(!$userid){
			return '';
		}
		$data			   =	array();
	...
	public function login() {
		if (IS_POST) {
			$mobile = $username = I ( 'username', '', 'htmlspecialchars' );
			// 测试账户
			...
			if(!$encrypt){
				$encrypt	=	$this->memberdb->where ( 'mobile="' . $mobile . '" AND groupid="' . $this->groupid . '" ' )->getField ( 'encrypt' );
	...
	public function register(){
		if(IS_POST){
			...
			if (!isset($userinfo['mobile']) || !trim($userinfo['mobile']) ||
					...
					if(\Bsugarmobile\Model\MemberModel::checkmobile($userinfo['mobile'])){
						$data['status'] = 7;
						$data['msg']	= '手机号已存在';
						echo $this->ajaxReturn($data);
						exit;
